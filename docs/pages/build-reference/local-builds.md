...
* goal
  * run EAS Build process, BUT LOCALLY

* `eas build --platform android --local` OR `eas build --platform ios --local`
  * use cases
    * debug build failures / happen | your cloud builds
  * ⚠️requirements⚠️
    * `eas login` OR set [`EXPO_TOKEN`](../accounts/programmatic-access.mdx)
    * specify `--platform`
      * != | EAS Build
...
If you encounter build failures on EAS servers and you're unable to determine the cause from inspecting the logs, you may find it helpful to debug the issue locally
* To simplify that process we support several environment variables to configure the local build process.
...
Some of the options available for cloud builds are not available locally
...
- You are responsible for making sure that the environment has all the necessary tools installed:
  ...
  - CocoaPods (iOS only)
  - Android SDK and NDK
- On Windows, you can use [WSL](https://learn.microsoft.com/en-us/windows/wsl/install) for local EAS Builds
  * However, we do not officially test against this platform and do not support Windows for local builds (macOS and Linux are supported).

## App compilation for development and production builds locally

To compile your app locally for development with Expo CLI, use `npx expo run:android` or `npx expo run:ios` commands instead
* If you use [Continuous Native Generation](/workflow/continuous-native-generation), you can also run [prebuild](/more/glossary-of-terms/#prebuild) to generate your **android** and **ios** directories and then proceed to open the projects in the respective IDEs and build them like any native project
...
To create a production build locally, you need Android Studio and Xcode installed on your computer
