...
> **warning** `createServerLoader` will throw an error if called during static site generation (SSG) because there is no HTTP request at build time. Use `createStaticLoader` when using static rendering.
